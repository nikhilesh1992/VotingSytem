Executing - 
Enter directory EXTRA
1. make clean
2. make
3. Start the server on one terminal. ( ./server-tcp-nonblocking )
4. Invoke various clients in any given order
	Client-1 : ./vote-zero-tcp-nonblocking localhost 11632
	Client-2 : ./add-voter-tcp-nonblocking localhost 11632 <VoterID>
	Client-3 : ./vote-tcp-nonblocking localhost 11632 <Candidate Name> <VoterID>
	Client-4 : ./list-candidates-rcp-nonblocking localhost 11632
	Client-5 : ./vote-count-tcp-nonblocking localhost 11632 <Candidate Name>

Testing:
For testing purposes I have created a script file that run the 5 clients simultaneously and we observe that the order in which the server handles the request and hence is a prove of select function working and handling the requests. In addition to that I have also printed the requests that are being handled on the server side just to make sure of the results.
To run the script
	1) chmod 777 test.sh
	2)./test.sh
	Note: Run the script many times to observe the results which have different orders at all times of execution.
...
Note: If any of these conditions are not met then a corresponding error message is thrown at the client side while parsing the command line data	

Port Numbers
1. Server - 11632
2. Client-1(vote-zero-tcp-nonblocking) - 11633
3. Client-2(add-voter-tcp-nonblocking) - 11634
4. Client-3(vote-tcp-nonblocking) - 11635
5. Client-4(list-candidates-tcp-nonblocking) - 11636
6. Client-5(vote-count-tcp-nonblocking) - 11637
...
Server Side(Function Calls) : Create Socket with type SOCK_STREAM(socket), Bind Socket(bind), Listen to client request(listen), Accept client connections only when there is a client request (select), Receive message from client(read), Send message to client(write) 
